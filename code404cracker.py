...
class Code404CrackerGUI:
    ...
    def _build_single_tab(self, parent):
        ...
        # ── CLUES panel: below INPUT on the LEFT side (per user sketch) ──
        # The decoded output gets decoded AGAIN here automatically.
        tk.Label(left,
                 text="┌─[ CLUES ] result decoded AGAIN — hidden layers appear here",
                 font=FONT_LABEL, bg=BG, fg=PURPLE).pack(anchor=tk.W, padx=8, pady=(4, 0))

        self.clues_text = scrolledtext.ScrolledText(
            left, font=("Consolas", 9), bg=BG_INPUT, fg=WHITE,
            insertbackground=GREEN, relief=tk.FLAT, wrap=tk.WORD,
            padx=10, pady=8, height=5, state=tk.DISABLED)
        self.clues_text.pack(fill=tk.X, padx=8, pady=(2, 4))
