...
def decode_caesar(s: str) -> str | None:
    ...
    common_words = {"the", "and", "for", "are", "but", "not", "you", "all",
                    ...
                    "www", "link", "find", "decode", "hidden", "hello",
                    "world", "this", "test", "from", "with", "have", "here"}

    # English letter frequency percentages (for scoring)
    _ENG_FREQ = {'e': 12.7, 't': 9.1, 'a': 8.2, 'o': 7.5, 'i': 7.0, 'n': 6.7,
                 's': 6.3, 'h': 6.1, 'r': 6.0, 'd': 4.3, 'l': 4.0, 'u': 2.8,
                 'c': 2.8, 'm': 2.4, 'w': 2.4, 'f': 2.2, 'y': 2.0, 'g': 2.0,
                 'p': 1.9, 'b': 1.5, 'v': 1.0, 'k': 0.8, 'j': 0.1, 'x': 0.1,
                 'q': 0.1, 'z': 0.1}
    ...
    def _english_score(text):
        """Score how English-like the text is (letter frequency match)."""
        letters = [c.lower() for c in text if c.isalpha()]
        if not letters:
            return 0
        return sum(_ENG_FREQ.get(c, 0) for c in letters) / len(letters)
    ...
    for shift in range(1, 26):
        result = _shift_text(s, shift)
        # Common words are the strongest signal
        words = set(result.lower().split())
        score = len(words & common_words) * 10.0
        # Letter frequency tiebreaker — real English uses common letters
        score += _english_score(result)
        if score > best_score:
            best_score = score
            best_shift = shift
            best_result = result

    if best_shift is not None and best_result:
...
def decode_url(s: str) -> str | None:
    s = _clean(s)
    # Must contain % or + to be URL encoded
    if "%" not in s and "+" not in s:
        return None
...
def decode_a1z26(s):
    ...
    for p in parts:
        ...
        if p.isdigit():
            ...
            result.append(p)
    if not result or not has_numbers:
        raise ValueError("Not valid A1Z26 (no numbers found)")
    return ''.join(result)
...
def encode_bcd(s):
    ...
    for c in s:
        if c.isdigit():
            ...
            skipped += 1
    if not result:
        return "[BCD only encodes digits 0-9 — no digits found in input]"
    output = ' '.join(result)
    if skipped:
        output += f"  [{skipped} non-digit characters skipped]"
    return output
...
INPUT_PATTERNS = {
    ...
    # --- v3.0 pattern matchers ---
    "Tap Code": re.compile(r'^[1-5],[1-5](\s+[1-5],[1-5])*$'),
    "A1Z26": re.compile(r'^([1-9]|1[0-9]|2[0-6])(\s+([1-9]|1[0-9]|2[0-6]))+$'),
    "Polybius Square": re.compile(r'^[1-5][1-5]( [1-5][1-5])+$'),
    "BCD": re.compile(r'^[01]{4}( [01]{4})*$'),
}
...
def detect_and_decode(s: str) -> list[tuple[str, str]]:
    ...
    def _score(item):
        ...
        if name in ("Caesar", "ROT13", "Atbash", "Vigenere", "Affine Cipher", "Beaufort Cipher"):
            common_words = {"flag", "ctf", "the", "and", "for", "http",
                            "key", "secret", "password", "hidden", "link",
                            "hello", "world", "this", "test", "with", "have",
                            "here", "from", "find", "decode", "code", "crack"}
            words = set(dl.split())
        ...
        # HEAVY penalty for non-printable / garbage characters
        non_printable = sum(1 for c in decoded if ord(c) < 32 or ord(c) > 126)
        if non_printable > 0:
            score -= 100

        # Bonus for English-readable results
...
def recursive_decode(s: str, max_depth: int = 15, _depth: int = 0,
    ...
    # Guard: short plain numbers (like BCD output "123") are final —
    # don't let Octal/ASCII re-decode them into single characters
    if _depth > 0 and len(s) <= 12 and s.strip().isdigit():
        result.final_output = s
        result.is_final_readable = True
        result.found_flag = flag
        result.found_url = url
...
def is_readable(text: str) -> bool:
    ...
    if alpha / len(text) > 0.6 and len(text) >= 2 and len(text) <= 15:
        return True
...
class Code404CrackerGUI:
    ...
    def _build_single_tab(self, parent):
        ...
        tk.Label(parent,
                 text="  ↔ drag left/right divider to resize panels  •  ↕ drag up/down divider to resize input/clues",
        ...
        # LEFT: vertical split — INPUT (top) + CLUES (bottom)
        # The divider between them can be dragged up/down to resize.
        left = tk.PanedWindow(paned, orient=tk.VERTICAL,
                              bg=BG_PANEL, sashwidth=4, sashrelief=tk.RAISED,
                              sashpad=0, handlesize=10)

        # ── Top pane: INPUT ──
        input_pane = tk.Frame(left, bg=BG)
        ...
        tk.Label(input_pane, text="└─ paste any encoded text above ↑",
                 font=FONT_SMALL, bg=BG, fg=GRAY).pack(anchor=tk.W, padx=8, pady=(2, 4))

        left.add(input_pane, minsize=150)

        # ── Bottom pane: CLUES (auto re-decode of the result) ──
        clues_pane = tk.Frame(left, bg=BG)

        tk.Label(clues_pane,
                 text="┌─[ CLUES ] result decoded AGAIN — hidden layers appear here",
                 font=FONT_LABEL, bg=BG, fg=PURPLE).pack(anchor=tk.W, padx=8, pady=(4, 2))

        self.clues_text = scrolledtext.ScrolledText(
            clues_pane, font=("Consolas", 9), bg=BG_INPUT, fg=WHITE,
            insertbackground=GREEN, relief=tk.FLAT, wrap=tk.WORD,
            padx=10, pady=8, height=5, state=tk.DISABLED)
        self.clues_text.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0, 2))
        self.clues_text.tag_config("clue_head", foreground=PURPLE,
                                   font=("Consolas", 9, "bold"))
        self.clues_text.tag_config("clue_name", foreground=CYAN,
                                   font=("Consolas", 9, "bold"))
        self.clues_text.tag_config("clue_val", foreground=WHITE,
                                   font=("Consolas", 9))
        self.clues_text.tag_config("clue_none", foreground=GRAY,
                                   font=("Consolas", 9, "italic"))

        tk.Label(clues_pane, text="└─ drag the divider above ↑ to resize this box",
                 font=FONT_SMALL, bg=BG, fg=GRAY).pack(anchor=tk.W, padx=8, pady=(2, 4))

        left.add(clues_pane, minsize=80)
        ...
        # Auto-detect on paste (must come after input_text/output_text exist)
        ...
        # Right-click menu for input and output
        self._add_right_click_menu(self.input_text)
        self._add_right_click_menu(self.output_text)

        # Set initial split (50/50) — wrapped in try/except because some
        # tkinter versions / wrappers don't support sashpos
        try:
            paned.sashpos(0, 580)
        except Exception:
            pass  # sash position is cosmetic; GUI works without it
    ...
    def _display_decode_result(self, result):
        ...
        # Auto re-decode the final result in the CLUES panel
        self._update_clues(result.final_output)

    def _update_clues(self, final_output):
        """Auto re-decode the decoded result to find hidden layers / clues."""
        if not hasattr(self, "clues_text"):
            return
        ...
        if not final_output or not str(final_output).strip():
            self.clues_text.insert(tk.END, "  nothing to analyze\n", "clue_none")
            self.clues_text.configure(state=tk.DISABLED)
            return

        text = str(final_output).strip()

        # Guard: skip gigantic outputs
        if len(text) > 5000:
            self.clues_text.insert(tk.END, "  result too large for clue analysis\n", "clue_none")
            self.clues_text.configure(state=tk.DISABLED)
            return

        try:
            results = detect_and_decode(text)
        except Exception:
            results = []

        if results:
            self.clues_text.insert(
                tk.END, "  HIDDEN LAYERS FOUND! The decoded result decodes AGAIN:\n\n", "clue_head")
            for name, decoded in results[:8]:
                preview = str(decoded)[:80] + ("..." if len(str(decoded)) > 80 else "")
                self.clues_text.insert(tk.END, f"  → {name}:  ", "clue_name")
                self.clues_text.insert(tk.END, f"{preview}\n", "clue_val")
        else:
            self.clues_text.insert(
                tk.END, "  No further encoding detected — the result looks like final plaintext.\n", "clue_none")
        self.clues_text.configure(state=tk.DISABLED)
